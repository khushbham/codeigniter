<?php if(!$item_verzonden): ?>
	<div id="berichten">
		<form method="post" action="<?php echo current_url() ?>">
			<?php if($item_beantwoorden): ?>
				...
				<div id="buttons">
					<p>
						...
						<a href="<?php echo base_url('cursistenmodule/berichten/'.$item_ID) ?>" title="Annuleren">Annuleren</a>
					</p>
				</div>
				<p><label>Aan</label><?php echo $bericht->gebruiker_naam ?></p>
				<input type="hidden" name="item_ontvanger[]" value="<?php echo $bericht->bericht_afzender_ID; ?>" required>
			<?php else: ?>
					<h1>Nieuw bericht</h1>
					<div id="buttons">
						<p>
							<input type="submit" name="item_verzenden" id="item_verzenden" value="Verzenden" class="belangrijk" />
							<a href="<?php echo base_url('cursistenmodule/berichten') ?>" title="Annuleren">Annuleren</a>
						</p>
					</div>
					<p>
						...
						<select multiple="multiple" name="item_ontvanger[]" id="item_ontvanger" required>
							...
							<?php if(sizeof($beheerders) > 0 && $login_user != 'deelnemer' && $login_user != 'kandidaat'): ?>
			...
			<?php endif; ?>
			<p><label for="item_onderwerp">Onderwerp</label>
			<input type="text" name="item_onderwerp" id="item_onderwerp" value="<?php echo $item_onderwerp ?>" autocomplete="off" required /></p>
...
<?php else: ?>
	<div id="item_verzonden">
		...
		<?php endif; ?>
		<p><a href="<?php echo base_url('cursistenmodule/berichten') ?>" title="Naar alle berichten">Naar alle berichten</a></p>
	</div>
<?php endif; ?>
