...
class Revuedesvoix extends CI_Controller
{
	private $data = array();
	...
	public function index()
	{
		...
		$content = $this->paginas_model->getPaginaByID(15);
		$content->video_url = $this->urlBouwen($content->video_url);

		if(!empty($content->chat_url)) {
			$content->chat_url = $this->urlbouwen($content->chat_url);
		}
		...
		$pagina['pagina'] = 'revue-des-voix';
		$this->load->view('template', $pagina);
	}

	private function urlBouwen($item_url) {
        $video_url = "";
        
        if(!empty($item_url)) {
			$url = explode('/', $item_url);
				$video_url = $url[3];
		}
