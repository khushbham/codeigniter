...
class Profiel extends CursistenController
{
	...
	public function index()
	{
		...
		$pagina['pagina'] = 'cursistenmodule/profiel';
	...
	public function wijzigen($wijzigen = null)
	{
        if ($this->session->userdata('demo') == true) {
            redirect('cursistenmodule/profiel');
        }
	...
	private function _gegevens_wijzigen()
	{
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			if($fouten == 0)
			{
				...
				if($update) redirect('cursistenmodule/profiel');
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'cursistenmodule/profiel_wijzigen_gegevens';
		$this->load->view('cursistenmodule/template', $pagina);
	}
	...
	private function _wachtwoord_wijzigen()
	{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						if($updaten) redirect('cursistenmodule/profiel');
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'cursistenmodule/profiel_wijzigen_wachtwoord';
		$this->load->view('cursistenmodule/template', $pagina);
	}
	...
	private function _instellingen_wijzigen()
	{
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'cursistenmodule/profiel_wijzigen_instellingen';
		$this->load->view('cursistenmodule/template', $pagina);
	}
