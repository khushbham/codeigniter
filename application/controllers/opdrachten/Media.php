...
class Media extends CI_Controller
{
	...
	public function pagina($p = 1, $media_soort = null)
	{
		...
		if($p > 1 && sizeof($media) == 0) redirect('opdrachten/media');
		...
		$pagina['pagina'] = 'opdrachten/media';
	...
	{
		if($item_ID == null) redirect('opdrachten/media');

		$this->load->model('media_model');
		$item = $this->media_model->getMediaByID($item_ID);
		if($item == null) redirect('opdrachten/media');
		$this->data['item'] = $item;
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'opdrachten/media_detail';
		$this->load->view('opdrachten/template', $pagina);
	}
			...
			else
			{
				...
				if($item_type == 'pdf')
				{
					...
					else
					{
						...
						if($q) redirect('opdrachten/media/'.$item_ID);
				...
				elseif($item_type == 'video')
				{
					...
					if($fouten == 0)
					{
						...
						if($q)
						{
							if($actie == 'toevoegen') redirect('opdrachten/media');
							else redirect('opdrachten/media/'.$item_ID);
						}
						else
						{
							echo 'Item '.$actie.' mislukt. Probeer het nog eens.';
	...
	else
	{
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'opdrachten/media_wijzigen';
		$this->load->view('opdrachten/template', $pagina);
	}
	...
	public function verwijderen($item_ID = null, $bevestiging = null)
	{
		...
		if($bevestiging == 'ja')
		{
			...
			if($q) redirect('opdrachten/media');
		...
		$pagina['data'] = $this->data;
		$pagina['pagina'] = 'opdrachten/media_verwijderen';
		$this->load->view('opdrachten/template', $pagina);
	}
}